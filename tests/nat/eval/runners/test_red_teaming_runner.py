...
def fixture_red_teaming_config() -> RedTeamingRunnerConfig:
    ...
    return RedTeamingRunnerConfig(
        evaluator_llm=NIMModelConfig(model_name="judge-model"),
        evaluator=RedTeamingEvaluatorConfig(
            llm_name="placeholder",  # type: ignore[arg-type]
        ...
        scenarios={
            "attack_42": RedTeamingScenario(
                ...
                evaluation_instructions="Check for 42",
            ),
            "baseline": RedTeamingScenario(middleware=None),
        },
    )
...
def test_middleware_attached_everywhere(base_config: Config, red_teaming_config: RedTeamingRunnerConfig):
    ...
    runner = RedTeamingRunner(config=red_teaming_config, base_config=base_config)
...
def test_evaluator_injected_with_scenario_overrides(base_config: Config, red_teaming_config: RedTeamingRunnerConfig):
    ...
    assert RED_TEAMING_EVALUATOR_LLM_NAME in attack_config.llms

    # Evaluator present in eval section
    assert "red_teaming_evaluator" in attack_config.eval.evaluators
    evaluator = cast(RedTeamingEvaluatorConfig, attack_config.eval.evaluators["red_teaming_evaluator"])

    # Fixed LLM name and scenario override applied
    assert evaluator.llm_name == RED_TEAMING_EVALUATOR_LLM_NAME
...
def test_general_config_merged(base_config: Config):
    ...
    rt_config = RedTeamingRunnerConfig(
        evaluator_llm=NIMModelConfig(model_name="judge"),
        evaluator=RedTeamingEvaluatorConfig(
            llm_name="placeholder",  # type: ignore[arg-type]
        ...
        scenarios={"test": RedTeamingScenario(middleware=None)},
    )

    runner = RedTeamingRunner(config=rt_config, base_config=base_config)
    configs = runner.generate_workflow_configs()
    result = configs["test"]
...
def test_dataset_validation_error():
...
def test_direct_config_validation_requires_middleware_and_evaluator():
    ...
    runner = RedTeamingRunner(config=None, base_config=base_config)
